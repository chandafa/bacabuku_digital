...
if (isset($_POST['submit'])) {
    ...
    // daftar pengguna, kata sandi, dan peran
    $admins = [
        'admin1' => ['password' => 'admin1', 'role' => 'admin'],
        'admin2' => ['password' => 'password2', 'role' => 'admin'],
        'admin3' => ['password' => 'password3', 'role' => 'admin'],
        // tambahkan pengguna biasa jika diperlukan
        'user1' => ['password' => 'userpass1', 'role' => 'user'],
        'user2' => ['password' => 'userpass2', 'role' => 'user']
    ];

    // validasi login benar atau salah
    if (array_key_exists($user, $admins) && $admins[$user]['password'] == $password) {
        // jika login benar maka user dan role akan disimpan ke session kemudian akan di redirect ke halaman profil
        $_SESSION['user'] = ['username' => $user, 'role' => $admins[$user]['role']];
...
<body class="text-center">
    <main class="form-signin">
        <form action="login.php" method="POST">
            ...
            <p class="my-3 text-muted">&copy; 2024</p>
        </form>
    </main>
